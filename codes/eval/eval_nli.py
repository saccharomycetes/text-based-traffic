...
def load_and_tokenize(args, tokenizer):
	with open(args.dev_file)as f_dev:
...
def convert_examples_to_features(args, examples, tokenizer):
    ...
    for i, example in tqdm(enumerate(examples),ncols=100):
        if args.data == "bdd":
            if args.test_type == "e":
                ids = [tokenizer.encode(example["c"], e + ".", return_tensors='pt', truncation=True).numpy().tolist()[0] for e in example["e"]]
            else:
                ids = [tokenizer.encode(c, example["e"] + ".", return_tensors='pt', truncation=True).numpy().tolist()[0] for c in example["c"]]
        else:
...
def main():
	...
	parser.add_argument("--model_type", default="microsoft/deberta-v2-xlarge-mnli", type=str,
						help="model to use")
	parser.add_argument("--test_type", default="e", type=str,
						help="type of testing data")
	args = parser.parse_args()

	args.output_dir = "../../results/nli/" + args.model_type.split("/")[-1]  + "/" + args.data + "_" + args.test_type
