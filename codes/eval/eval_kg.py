...
class t5_eval():
    ...
    def __init__(self, args):
        ...
        self.tokenizer = T5Tokenizer.from_pretrained(args.model, do_lower_case=True)
        ...
        for example in examples:
            if self.args.data == "bdd":
                if self.args.test_type == "e":
                    inputs = ["reasoning:  " + example["c"] + " " + e + "." for e in example["e"]]
                else:
                    inputs = ["reasoning:  " + c + " " + example["e"] + "." for c in example["c"]]
            else:
    ...
    def load_and_tokenize(self):
        with open(self.args.dev_file)as f_dev:
    ...
    def evaluate(self):
        ...
        with open(output_acc_file, "w") as writer:
            print("***** Eval results *****")
            print("model:" + self.args.model_type)
...
class roberta_eval():
    ...
    def convert_examples_to_features(self, examples):
        data = []
        for example in examples:
            if self.args.test_type == "c":
                inputs, labels = self.handle_underscores(example['e'], prefix=True)
                choices = [self.handle_underscores(cand) for cand in example['c']]
                input_ids = [cand[0]+inputs for cand in choices]
                input_ids = [self.tokenizer.convert_tokens_to_ids(cand) for cand in input_ids]
                label_ids = [cand[1]+labels for cand in choices]
            else:
                if self.args.test_type == "e":
                    con = example['c']
                    cands = example['e']
            ...
            dimension = self.args.data
            sample_id = example["id"]
            data.append([input_ids, label_ids, example['answer'], dimension, sample_id])	
...
def main():
	...
	parser.add_argument("--data", default="bdd", type=str, 
                        help="The data name")
	parser.add_argument("--eval_batch_size", default=8, type=int,
						help="Batch size per GPU/CPU for evaluation.")
	parser.add_argument("--model_type", default="t5-small", type=str,
						help="model to use")
	parser.add_argument("--test_type", default="1", type=str,
						help="type of testing data")
	args = parser.parse_args()

	args.output_dir = "../../results/kg/" + args.model_type + "/" + args.data + "_" + args.test_type
	args.dev_file = "../../data/" + args.data + "/" + args.test_type + ".jsonl"
	args.model = "../../models/kg/" + args.model_type
	...
	model = t5_eval(args) if args.model_type.lower().find("t5") != -1 else roberta_eval(args)
